...
RBM::RBM(uint16_t in_visible_count, uint16_t in_hidden_count, bool in_visible_units_linear) : _visible_means(0), _visible_stddev(0)
{
...
template<typename T>
void write(FILE* File, const T* Value, uint32_t Count=1)
{
	fwrite(Value, sizeof(T), Count, File);
}

template<typename T>
void read(FILE* File, T* Value, uint32_t Count = 1)
{
	fread(Value, sizeof(T), Count, File);
}

bool RBM::Save(const char* filename)
{
...
RBM* RBM::Load(const char* filename)
{
	RBM* result = NULL;
